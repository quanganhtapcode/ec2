...
# Configure CORS with specific allowed origins (more secure than wildcard)
ALLOWED_ORIGINS = [
    'https://valuation.quanganh.org',
...
def add_security_headers(response):
    # CORS headers (ensure they're present even if flask-cors fails)
...
DOWNLOAD_COOLDOWN = 60  # Cooldown period after hitting limit (in seconds)
...
class StockDataProvider:
    ...
    def _search_symbol_in_all_industries(self, symbol: str) -> dict:
        """Search for a symbol by looking for its individual JSON file in stock_data folder"""
        symbol_upper = symbol.upper()
        ...
        try:
            ...
            if os.path.exists(stock_file):
                ...
                try:
                    ...
                    logger.error(f"Error reading {stock_file}: {e}")
                    return {}
            else:
                logger.info(f"✗ File {stock_file} not found")
                return {}
            
        except Exception as e:
            logger.error(f"Error searching for symbol {symbol_upper}: {e}")
            return {}
    ...
    def _get_company_info_from_csv(self, symbol: str) -> dict:
        """Get company info - returns defaults, CSV is not used anymore"""
        # CSV is no longer used for lookup, only JSON files
        return {
    ...
    def get_stock_data(self, symbol: str, period: str = "year", fetch_current_price: bool = False, symbols_override=None) -> dict:
        """Get stock data by searching directly in JSON files, then fallback to vnstock API if not found"""
        symbol = symbol.upper()
        
        if period == "quarter":
            # For quarter data, always fetch from vnstock API
            logger.info(f"Getting latest quarter data for {symbol} from vnstock API")
        ...
        # For annual data, search directly in stock_data folder
        logger.info(f"Looking for {symbol}.json in stock_data folder...")
        annual_data = self._search_symbol_in_all_industries(symbol)
    ...
    try:
        ...
        if cache_key in valuation_cache:
            ...
            if time.time() - cached_data['timestamp'] < VALUATION_CACHE_DURATION:
                ...
                valid_models = {k: results['valuations'][k] for k in ['fcfe', 'fcff', 'justified_pe', 'justified_pb'] 
                               if results['valuations'][k] > 0}
                
                if valid_models:
                    total_weight = sum(model_weights[k] for k in valid_models.keys())
                    if total_weight > 0:
                        results['valuations']['weighted_average'] = sum(
        ...
        if financial_data_from_request:
            ...
            logger.info(f"Financial data not in request, fetching from vnstock (slow)")
            try:
        ...
        if not current_price:
            ...
            logger.info(f"Using current price from request for {symbol}: {current_price}")
        ...
        # Cache the results (without weights, as they can be recalculated quickly)
...
# Explicit CORS preflight handler for valuation endpoint
@app.route("/api/valuation/<symbol>", methods=['OPTIONS'])
def cors_preflight_valuation(symbol):
    response = app.make_default_options_response()
    return response

# Ensure CORS headers are present on all API responses (belt-and-suspenders)
@app.after_request
...
def get_download_stats():
    ...
    try:
        current_time = time.time()
        stats = {
            "total_ips": len(download_tracker),
            "rate_limit_config": {
                "max_downloads_per_hour": DOWNLOAD_LIMIT,
