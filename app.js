...
Generated by Stock Valuation Tool
    ...
    async exportExcelReport() {
        ...
        try {
            ...
            // SHEET 1: Detailed Report (CSV-style with step-by-step calculations)
            const reportSheet = workbook.addWorksheet('Valuation Report', {
                views: [{ showGridLines: false }]
            });
            
            this.createDetailedReportSheet(reportSheet);
            
            // SHEET 2: Excel Data (structured data for Excel formulas)
            const dataSheet = workbook.addWorksheet('Data & Calculations', {
                views: [{ showGridLines: true }]
            });
            
            this.createExcelDataSheet(dataSheet);
            ...
            if (typeof saveAs !== 'undefined') {
                saveAs(blob, `${this.currentStock}_Valuation_Report_${new Date().toISOString().split('T')[0]}.xlsx`);
            } else {
                // Fallback if FileSaver not loaded
                const url = URL.createObjectURL(blob);
                const a = document.createElement('a');
                a.href = url;
                a.download = `${this.currentStock}_Valuation_Report_${new Date().toISOString().split('T')[0]}.xlsx`;
                document.body.appendChild(a);
                a.click();
                document.body.removeChild(a);
                URL.revokeObjectURL(url);
            ...
            this.showStatus('Excel report generated and downloaded successfully!', 'success');
        } catch (error) {
