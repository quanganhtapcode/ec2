...
Generated by Stock Valuation Tool
    ...
    exportExcelReport() {
        ...
        try {
            ...
            const a = document.createElement('a');
            a.href = url;
            a.download = `${this.currentStock}_Valuation_Report_${new Date().toISOString().split('T')[0]}.csv`;
            a.style.display = 'none';
            document.body.appendChild(a);
            a.click();
            document.body.removeChild(a);
            ...
            this.showStatus('Excel report (CSV) generated and downloaded successfully!', 'success');
        } catch (error) {
    ...
    generateCSVReport() {
        ...
        csv.push(`"${t.companyInformation || 'Company'}","${this.stockData.name} (${this.currentStock})"`);
        csv.push(`"${t.reportDate || 'Report Date'}","${new Date().toLocaleDateString(lang === 'vi' ? 'vi-VN' : 'en-US')}"`);
        csv.push('');
        
        // Company Information
        csv.push(`"${t.companyInformation || 'COMPANY INFORMATION'}"`);
        csv.push(`"${t.symbol || 'Stock Symbol'}","${this.stockData.symbol || '--'}"`);
        csv.push(`"${t.name || 'Company Name'}","${escapeCSV(this.stockData.name || '--')}"`);
        csv.push(`"${t.industry || 'Industry'}","${escapeCSV(this.stockData.sector || '--')}"`);
        csv.push(`"${t.exchange || 'Exchange'}","${this.stockData.exchange || '--'}"`);
        csv.push('');
        
        // Market Data
        csv.push(`"${t.marketData || 'MARKET DATA'}"`);
        csv.push(`"${t.currentPrice || 'Current Price'}","${this.formatCurrency(currentPrice)}"`);
        csv.push(`"${t.marketCap || 'Market Cap'}","${this.formatLargeNumber(this.stockData.market_cap)}"`);
        csv.push(`"${t.sharesOutstanding || 'Shares Outstanding'}","${this.formatLargeNumber(this.stockData.shares_outstanding)}"`);
        csv.push(`"${t.peRatio || 'P/E Ratio'}","${this.formatNumber(this.stockData.pe_ratio)}"`);
        csv.push(`"${t.pbRatio || 'P/B Ratio'}","${this.formatNumber(this.stockData.pb_ratio)}"`);
        csv.push(`"${t.eps || 'EPS'}","${this.formatCurrency(this.stockData.eps)}"`);
        csv.push(`"${t.bookValuePerShare || 'Book Value/Share'}","${this.formatCurrency(this.stockData.book_value_per_share)}"`);
        csv.push('');
        
        // Valuation Results
        csv.push(`"${t.valuationResults || 'VALUATION RESULTS'}"`);
        csv.push(`"${t.valuationModel || 'Valuation Model'}","${t.shareValue || 'Share Value (VND)'}","${t.weight || 'Weight'}","${t.formula || 'Formula/Method'}"`);
        
        // FCFE
        const fcfe = this.valuationResults.fcfe;
        csv.push(`"${t.fcfe || 'FCFE'} (${t.fcfeDesc || 'Free Cash Flow to Equity'})","${this.formatCurrency(fcfe.shareValue)}","${this.modelWeights.fcfe}%","PV of projected FCFE / Shares Outstanding"`);
        if (fcfe.projectedCashFlows && fcfe.projectedCashFlows.length > 0) {
            csv.push(`"","${t.projectedCashFlows || 'Projected Cash Flows'}"`);
            fcfe.projectedCashFlows.forEach((cf, idx) => {
                csv.push(`"","${t.year || 'Year'} ${idx + 1}","${this.formatLargeNumber(cf)}"`);
            });
            csv.push(`"","${t.terminalValue || 'Terminal Value'}","${this.formatLargeNumber(fcfe.terminalValue || 0)}"`);
            csv.push(`"","${t.totalPV || 'Total Present Value'}","${this.formatLargeNumber(fcfe.equityValue || 0)}"`);
        }
        csv.push('');
        
        // FCFF
        const fcff = this.valuationResults.fcff;
        csv.push(`"${t.fcff || 'FCFF'} (${t.fcffDesc || 'Free Cash Flow to Firm'})","${this.formatCurrency(fcff.shareValue)}","${this.modelWeights.fcff}%","(PV of FCFF - Net Debt) / Shares"`);
        if (fcff.projectedCashFlows && fcff.projectedCashFlows.length > 0) {
            csv.push(`"","${t.projectedCashFlows || 'Projected Cash Flows'}"`);
            fcff.projectedCashFlows.forEach((cf, idx) => {
                csv.push(`"","${t.year || 'Year'} ${idx + 1}","${this.formatLargeNumber(cf)}"`);
            });
            csv.push(`"","${t.terminalValue || 'Terminal Value'}","${this.formatLargeNumber(fcff.terminalValue || 0)}"`);
            csv.push(`"","${t.enterpriseValue || 'Enterprise Value'}","${this.formatLargeNumber(fcff.enterpriseValue || 0)}"`);
            csv.push(`"","${t.netDebt || 'Net Debt'}","${this.formatLargeNumber(fcff.netDebt || 0)}"`);
            csv.push(`"","${t.equityValue || 'Equity Value'}","${this.formatLargeNumber(fcff.equityValue || 0)}"`);
        }
        csv.push('');
        
        // P/E
        const pe = this.valuationResults.justified_pe;
        csv.push(`"${t.justifiedPE || 'Justified P/E'}","${this.formatCurrency(pe.shareValue)}","${this.modelWeights.justified_pe}%","Justified P/E × Current EPS"`);
        csv.push(`"","${t.justifiedPERatio || 'Justified P/E Ratio'}","${this.formatNumber(pe.ratio)}"`);
        csv.push(`"","${t.currentEPS || 'Current EPS'}","${this.formatCurrency(this.stockData.eps)}"`);
        csv.push(`"","${t.formula || 'Formula'}","Payout Ratio × (1 + g) / (r - g)"`);
        csv.push('');
        
        // P/B
        const pb = this.valuationResults.justified_pb;
        csv.push(`"${t.justifiedPB || 'Justified P/B'}","${this.formatCurrency(pb.shareValue)}","${this.modelWeights.justified_pb}%","Justified P/B × Book Value/Share"`);
        csv.push(`"","${t.justifiedPBRatio || 'Justified P/B Ratio'}","${this.formatNumber(pb.ratio)}"`);
        csv.push(`"","${t.bookValuePerShare || 'Book Value/Share'}","${this.formatCurrency(this.stockData.book_value_per_share)}"`);
        csv.push(`"","${t.formula || 'Formula'}","ROE × Payout Ratio × (1 + g) / (r - g)"`);
        csv.push('');
        
        // Weighted Average
        csv.push(`"${t.weightedAverageTargetPrice || 'WEIGHTED AVERAGE TARGET PRICE'}","${this.formatCurrency(weightedValue)}"`);
        csv.push('');
        
        // Market Comparison
        csv.push(`"${t.marketComparison || 'MARKET COMPARISON'}"`);
        csv.push(`"${t.currentPrice || 'Current Market Price'}","${this.formatCurrency(currentPrice)}"`);
        csv.push(`"${t.targetPrice || 'Intrinsic Value (Target)'}","${this.formatCurrency(weightedValue)}"`);
        csv.push(`"${t.upsideDownside || 'Upside/Downside Potential'}","${upside >= 0 ? '+' : ''}${upside.toFixed(2)}%"`);
        
        if (this.valuationResults.market_comparison?.recommendation) {
            const rec = this.valuationResults.market_comparison.recommendation;
            csv.push(`"${t.recommendation || 'Investment Recommendation'}","${rec.toUpperCase()}"`);
        }
        csv.push('');
        
        // Valuation Assumptions
        csv.push(`"${t.modelAssumptions || 'VALUATION ASSUMPTIONS'}"`);
        csv.push(`"${t.revenueGrowth || 'Revenue Growth Rate'}","${this.assumptions.revenueGrowth}%"`);
        csv.push(`"${t.terminalGrowth || 'Terminal Growth Rate'}","${this.assumptions.terminalGrowth}%"`);
        csv.push(`"${t.wacc || 'WACC (Cost of Capital)'}","${this.assumptions.wacc}%"`);
        csv.push(`"${t.requiredReturn || 'Required Return (Equity)'}","${this.assumptions.requiredReturn}%"`);
        csv.push(`"${t.taxRate || 'Corporate Tax Rate'}","${this.assumptions.taxRate}%"`);
        csv.push(`"${t.projectionYears || 'Projection Period'}","${this.assumptions.projectionYears} ${t.years || 'years'}"`);
        csv.push('');
        
        // Model Weights
        csv.push(`"${t.modelWeights || 'MODEL WEIGHTS'}"`);
        csv.push(`"${t.fcfe || 'FCFE'} ${t.weight || 'Weight'}","${this.modelWeights.fcfe}%"`);
        csv.push(`"${t.fcff || 'FCFF'} ${t.weight || 'Weight'}","${this.modelWeights.fcff}%"`);
        csv.push(`"${t.justifiedPE || 'Justified P/E'} ${t.weight || 'Weight'}","${this.modelWeights.justified_pe}%"`);
        csv.push(`"${t.justifiedPB || 'Justified P/B'} ${t.weight || 'Weight'}","${this.modelWeights.justified_pb}%"`);
        csv.push('');
        
        // Financial Metrics
        csv.push(`"${t.financialMetrics || 'KEY FINANCIAL METRICS'}"`);
        csv.push(`"${t.revenue || 'Revenue (TTM)'}","${this.formatLargeNumber(this.stockData.revenue_ttm)}"`);
        csv.push(`"${t.netIncome || 'Net Income (TTM)'}","${this.formatLargeNumber(this.stockData.net_income_ttm)}"`);
        csv.push(`"${t.ebitda || 'EBITDA'}","${this.formatLargeNumber(this.stockData.ebitda)}"`);
        csv.push(`"${t.roe || 'ROE (Return on Equity)'}","${this.formatPercent(this.stockData.roe)}"`);
        csv.push(`"${t.roa || 'ROA (Return on Assets)'}","${this.formatPercent(this.stockData.roa)}"`);
        csv.push(`"${t.debtEquity || 'Debt/Equity Ratio'}","${this.formatNumber(this.stockData.debt_to_equity)}"`);
        csv.push('');
        
        // Disclaimer
        csv.push(`"${t.disclaimer || 'DISCLAIMER'}"`);
        csv.push(`"${t.disclaimerText || 'This report is for informational purposes only and does not constitute investment advice. Past performance does not guarantee future results. Please consult with a qualified financial advisor before making investment decisions.'}"`);
        csv.push('');
        csv.push(`"${t.generatedBy || 'Generated by Stock Valuation Tool'}","${new Date().toLocaleString(lang === 'vi' ? 'vi-VN' : 'en-US')}"`);
