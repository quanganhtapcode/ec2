...
Generated by Stock Valuation Tool
    ...
    createFCFESheet(sheet, stockData, valuationResults, assumptions) {
        ...
        let cf = fcfeData.projectedCashFlows || [];
        if (cf.length > 0) {
            ['Year', 'Projected FCFE', 'Discount Factor', 'Present Value'].forEach((h, i) => {
                sheet.getCell(row, i + 1).value = h;
                sheet.getCell(row, i + 1).font = { bold: true };
        ...
        sheet.getCell(`A${row}`).value = 'Terminal Value = FCFEₙ × (1 + g) / (r - g)';
        sheet.getCell(`A${row}`).font = { italic: true, color: { argb: '6B7280' } };
        row++;

        sheet.getCell(`A${row}`).value = 'Terminal Value';
        sheet.getCell(`B${row}`).value = fcfeData.terminalValue || 0;
        sheet.getCell(`B${row}`).numFmt = '#,##0';
        row++;

        sheet.getCell(`A${row}`).value = 'PV of Terminal Value';
        sheet.getCell(`B${row}`).value = fcfeData.pvTerminal || (fcfeData.terminalValue / Math.pow(1 + (assumptions.requiredReturn || 12) / 100, assumptions.projectionYears || 5));
        sheet.getCell(`B${row}`).numFmt = '#,##0';
        row += 2;
        ...
        sheet.getCell(`B${row}`).value = fcfeData.equityValue || 0;
        sheet.getCell(`B${row}`).numFmt = '#,##0';
        sheet.getCell(`C${row}`).value = 'VND';
        row++;

        sheet.getCell(`A${row}`).value = 'Fair Value per Share';
        sheet.getCell(`B${row}`).value = fcfeData.shareValue || valuationResults.fcfe || 0;
        sheet.getCell(`B${row}`).numFmt = '#,##0';
        ...
        const upside = stockData.current_price ? ((fcfeData.shareValue - stockData.current_price) / stockData.current_price) : 0;
    ...
    createFCFFSheet(sheet, stockData, valuationResults, assumptions) {
        ...
        let cf = fcffData.projectedCashFlows || [];
        if (cf.length > 0) {
            ['Year', 'Projected FCFF', 'Discount Factor', 'Present Value'].forEach((h, i) => {
                sheet.getCell(row, i + 1).value = h;
                sheet.getCell(row, i + 1).font = { bold: true };
        ...
        sheet.getCell(`A${row}`).value = 'Terminal Value = FCFFₙ × (1 + g) / (WACC - g)';
        sheet.getCell(`A${row}`).font = { italic: true, color: { argb: '6B7280' } };
        row++;

        sheet.getCell(`A${row}`).value = 'Terminal Value';
        sheet.getCell(`B${row}`).value = fcffData.terminalValue || 0;
        sheet.getCell(`B${row}`).numFmt = '#,##0';
        row++;

        sheet.getCell(`A${row}`).value = 'PV of Terminal Value';
        sheet.getCell(`B${row}`).value = fcffData.pvTerminal || 0;
        sheet.getCell(`B${row}`).numFmt = '#,##0';
        row += 2;
        ...
        sheet.getCell(`B${row}`).value = fcffData.enterpriseValue || 0;
        sheet.getCell(`B${row}`).numFmt = '#,##0';
        sheet.getCell(`C${row}`).value = 'VND';
        row++;

        sheet.getCell(`A${row}`).value = '− Net Debt';
        sheet.getCell(`B${row}`).value = fcffData.netDebt || ((stockData.total_debt || 0) - (stockData.cash || 0));
        sheet.getCell(`B${row}`).numFmt = '#,##0';
        sheet.getCell(`C${row}`).value = 'VND';
        row++;

        sheet.getCell(`A${row}`).value = '= Equity Value';
        sheet.getCell(`B${row}`).value = fcffData.equityValue || 0;
        sheet.getCell(`B${row}`).numFmt = '#,##0';
        sheet.getCell(`C${row}`).value = 'VND';
        sheet.getCell(`B${row}`).font = { bold: true };
        row++;
        ...
        sheet.getCell(`A${row}`).value = 'Fair Value per Share';
        sheet.getCell(`B${row}`).value = fcffData.shareValue || valuationResults.fcff || 0;
        sheet.getCell(`B${row}`).numFmt = '#,##0';
        ...
        const upside = stockData.current_price ? ((fcffData.shareValue - stockData.current_price) / stockData.current_price) : 0;
        sheet.getCell(`A${row}`).value = 'Upside/Downside';
        sheet.getCell(`B${row}`).value = upside;
        sheet.getCell(`B${row}`).numFmt = '+0.00%;-0.00%';
        sheet.getCell(`B${row}`).font = { bold: true, color: { argb: upside >= 0 ? '28A745' : 'DC3545' } };
